...
<main id="main">
  ...
  <c:set var="activeLnb" value="stra" scope="request" />
  <div class="lnb-layout">
    ...
    <div class="lnb-content">
      ...
      <div class="bbsCard">
        <div class="bbsCard-grid">
          <c:if test="${empty noticeList and empty list}">
            <div class="bbsCard-empty">등록된 게시물이 없습니다.</div>
          </c:if>
          <c:forEach var="row" items="${list}">
            <div class="bbsCard-item" onclick="fnGoDetail('${row.pstCd}')">
              <c:choose>
                <c:when test="${not empty row.thumbUrl}">
                  <div class="bbsCard-thumb"><img src="${row.thumbUrl}" alt=""></div>
                </c:when>
                <c:otherwise>
                  <div class="bbsCard-thumbEmpty"><i class="bi bi-image"></i></div>
                </c:otherwise>
              </c:choose>
              <div class="bbsCard-body">
                <h4 class="bbsCard-title">${row.pstNm}</h4>
                <div class="bbsCard-meta">
                  <span>${row.rgtUsrNm}</span><span>${row.rgtDtm}</span><span>조회 ${row.viewCnt}</span>
                </div>
              </div>
            </div>
          </c:forEach>
        ...
        <div class="bbsCard-paging">
          <ul class="bbsCard-pages">
      ...
      <form id="pagingForm" action="${ctx}${listUrl}" method="post">
        <input type="hidden" name="brdCd" value="${brdCd}" />
...
<script>
  ...
  function fnGoDetail(pstCd) { $('#detailPstCd').val(pstCd); $('#goDetailForm').submit(); }
</script>
